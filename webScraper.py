...
class Article: #
    def __init__(self, title, content, site, date, comments):
        self.title = title
...
class NewsScraper:
    def NewsScraper_Mandiner(self): #Lescrapeli az adatokat az oldalról, majd beleírja a .csv fájlba
        ...
        for div in mainPage_soup.findAll('div', attrs={'class':'title'}):
            if(articleIndex<50):
                ...
                seged=title.get_text().split('|')
                site_title=seged[1]
                #cikk dátumának tárolása
                date= article_soup.find('div', class_='info')
                ...
                dict = {'site':site_title, 'title':article_title, 'date':article_date , 'content': article_content, 'comments':"", 'label': 0}
    ...
    def NewsScraper_hvg(self): #Lescrapeli az adatokat az oldalról, majd beleírja a .csv fájlba
        ...
        for div in mainPage_soup.findAll('div', attrs={'class':'title'}):
            if(articleIndex<10):
                ...
                article_title = div.find('a') #cikk címe scrape-elve, eltárolva title változóba
                print('1. cikk cím: ')
                print(article_title.get_text())
                #Belép minden cikkbe linken keresztül és kiszedi a cikk tartalmát + kommenteket
                #URL checker
                if ("https" not in div.find('a')['href']):
                    article_url = "https://hvg.hu/" + div.find('a')['href'] #cikk URL
                else:
                    article_url = div.find('a')['href']
